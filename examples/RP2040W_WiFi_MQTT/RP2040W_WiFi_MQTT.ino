...
void publishMQTT()
{
  ...
  {
    //Serial.println("Failed to send value to Temperature feed!");
    Serial.print("T");        // T means publishing OK
  }
  else
  {
    //Serial.println("Value to Temperature feed sucessfully sent!");
...
void createNewInstances()
{
  ...
  if (!mqtt)
  {
    ...
    if (mqtt)
    {
      ...
      Serial.print("AIO_SERVER = "); Serial.print(AIO_SERVER); Serial.print(", AIO_SERVERPORT = "); Serial.println(AIO_SERVERPORT);
      Serial.print("AIO_USERNAME = "); Serial.print(AIO_USERNAME); Serial.print(", AIO_KEY = "); Serial.println(AIO_KEY);
    }
    else
  ...
  if (!Temperature)
  {
    ...
    Serial.print("Creating new MQTT_Pub_Topic,  Temperature = "); Serial.println(completePubTopic);

    if (Temperature)
    {
      Serial.println("Creating new Temperature object OK");
      Serial.print("Temperature MQTT_Pub_Topic = "); Serial.println(completePubTopic);
    }
    else
  ...
  if (!LED_Control)
  {
    ...
    Serial.print("Creating new AIO_SUB_TOPIC, LED_Control = "); Serial.println(completeSubTopic);

    if (LED_Control)
    {
      Serial.println("Creating new LED_Control object OK");
      Serial.print("LED_Control AIO_SUB_TOPIC = "); Serial.println(completeSubTopic);
...
void setup()
{
  ...
  Serial.print("\nStart RP2040W_WiFi_MQTT on "); Serial.print(BOARD_NAME);
